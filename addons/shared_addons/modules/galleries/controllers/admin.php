...
class Admin extends Admin_Controller
{
	...
	public function index($id = NULL)
	{
        ...
        {
            // Get child galleries
            $galleries = $this->gallery_m->get_all($id);
            $sort = TRUE;
        }
        ...
        foreach ($galleries as $key => $gallery) {
            ...
            foreach ($folders as $folder) {
                if($folder->parent_id == $gallery->folder_id)
                {
                    array_push($child,$folder->id);
                }
            }
            
            $galleries[$key]->has_child = $child;
        }
	...
	public function manage($id)
	{
		...
		$thumbnails		= $this->gallery_image_m->where('gallery_images.gallery_id', $id)->dropdownlist('files.id', 'files.name');
     ...
     * @return  void
     */
    public function ajax_gallery_order_update()
    {
         
        $ids = explode(',', $this->input->post('order'));
        
        $result = $this->gallery_m->set_sort_order($ids);
