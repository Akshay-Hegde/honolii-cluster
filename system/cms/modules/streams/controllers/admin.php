...
class Admin extends Admin_Controller {
    ...
    function index()
    {
		...
		$this->data->streams = $this->streams_m->get_streams( $this->settings->item('records_per_page'), $this->uri->segment(4) );
	...
	function manage()
	{
		...
		// Get DB table name
		$this->data->table_name = PYROSTREAMS_DB_PRE.STR_PRE.$this->data->stream->stream_slug;
		
		// Get the table data
		$obj = $this->db->query("SHOW TABLE STATUS LIKE '{$this->data->table_name}'");
		$info = $obj->result();
		$info = $info[0];
		...
		if(!$info->Update_time):
			...
			$this->data->last_updated = $info->Create_time;
		
		else:

			$this->data->last_updated = $info->Update_time;
		...
		$this->template->build('admin/streams/manage/index', $this->data);
	}
	...
	function add()
	{
        ...
        $this->template
        		...
        		->append_metadata( js('new_stream.js','streams') )
	...
	function delete()
	{
		...
		$this->data->total_fields = $this->streams_m->count_stream_entries( $this->data->stream->stream_slug );
	...
	function assignments()
	{
	    ...
	    $this->template->append_metadata( js('assignment_sorting.js', 'streams') );
	...
	private function _manage_fields()
	{
        ...
        $this->db->where('stream_id', $this->data->stream_id);
        $obj = $this->db->get(ASSIGN_TABLE);
